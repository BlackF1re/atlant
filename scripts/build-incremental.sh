...
image() {
  mkdir -p "$DIR"
  rm -f "$DIR"/*.img "$DIR"/*.deb "$DIR"/*.tar.zst "$DIR"/*.packages.tsv "$DIR"/*.snapshot.txt \
    "$DIR"/RELEASE-METADATA.json "$DIR"/SHA256SUMS
  [[ -d $ROOT/out/rootfs && -d $ROOT/out/rootfs-nand ]] || rootfs
  [[ -s $ROOT/out/boot/zImage && -s $ROOT/out/boot/devicetree.dtb ]] || kernel

  # Build the NAND payload before the SD image, then embed the exact checksummed
  # payload into the normal SD rootfs. The resulting single .img is both the
  # ordinary SD installation and the NAND installer/recovery medium.
  bootloader
  packages
  nand_products
  embed_nand
